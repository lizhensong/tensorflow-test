...
def hidden_layer(input_tensor, regularizer):
    # 使用get_variable()函数会在训练神经网络时创建这些变量,而在测试过程中通过保存的模型加载这些变量的取值，
    # 在测试过程中可以在加载变量时将滑动平均变量重命名，这样就会在测试过程中使用变量的滑动平均值
...
with tf.Session() as sess:
    ...
    for i in range(max_steps):
        ...
        if i % 1000 == 0:
            ...
            saver.save(sess, "./mnist_model/mnist_model.ckpt",
