...
label = []
for i in range(200):
    # uniform从一个均匀分布[low,high)中随机采样。
    x1 = np.random.uniform(-1, 1)
    x2 = np.random.uniform(0, 2)
    # 这里对产生的x1和x2进行判断，如果产生的点落在半径为1的圆内，则label值为0
    # 否则label值为1
    # normal从正态分布中随机取样。第一个参数均值（正态分布的中心），第二个参数为标准差（越小越瘦高）
...
# 定义完成前向传播的隐层
def hidden_layer(input_tensor, weight_1, bias_1, weight_2, bias_2, weight_3, bias_3):
    layer1 = tf.nn.relu(tf.matmul(input_tensor, weight_1)+bias_1)
    layer2 = tf.nn.relu(tf.matmul(layer1, weight_2) + bias_2)
    return tf.matmul(layer2, weight_3)+bias_3
...
# 定义权重参数和偏置参数
# truncated_normal生成正态分布的随机数（如果随机出来的值超过平均值俩个标准差，这个数将被重新随机）
# stddev标准差0.1
...
with tf.Session() as sess:
    ...
    for i in range(training_steps):
        a = sess.run(train_op, feed_dict={x: data, y_: label})
        # 训练30000轮，但每隔2000轮就输出一次loss的值
        if i % 2000 == 0:
            loss_value = sess.run(loss, feed_dict={x: data, y_: label})
            print("After {} steps, loss_value is: {}" .format(i, loss_value))
