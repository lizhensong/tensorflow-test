...
# 定义存储训练轮数的变量，在使用Tensorflow训练神经网络时，一般会将代表训练轮数的变量通过trainable参数设置为不可训练的
...
x = tf.placeholder(tf.float32, [None, 784])   # INPUT_NODE=784
y_ = tf.placeholder(tf.float32, [None, 10])   # OUT_PUT=10
...
y = hidden_layer(x, weights1, biases1, weights2, biases2)
...
average_y = hidden_layer(x, averages_class.average(weights1), averages_class.average(biases1),
                         averages_class.average(weights2), averages_class.average(biases2))
...
regularizer = tf.contrib.layers.l2_regularizer(0.001)        # 计算L2正则化损失函数
...
with tf.Session() as sess:
    ...
    for i in range(max_steps):
        xs, ys = mnist.train.next_batch(batch_size=512)
    ...
    print("After {} trainging step(s) ,test accuracy using average model is {}%".format(max_steps, test_accuracy*100))
